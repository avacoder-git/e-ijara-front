...
class LandGeometryController extends Controller
{
    ...
    public function index(Request $request, District $district)
    {
        ...
        switch ($request->status) {
            case 1:
                $where = " and l.status_id = 1 and l.parent_id is null  and not exists  (select parent_id from lands  where parent_id = l.id) ";
                if ($appliedLands)
                    $where .= "and l.id not in ($appliedLands)";
                else
                    $where .= "and l.id not in (0)";
                break;
            case 2:
