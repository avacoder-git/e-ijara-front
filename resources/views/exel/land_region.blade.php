<table>
    ...
    <tr>
        ...
        @php
            ...
            $land_counts = \App\Services\StatHelper::getLandCountByRegion($region_id->regionid);
                           $land_suvli = \App\Services\StatHelper::GetSuvli($region_id->regionid);

        @endphp
        <td>{{\App\Models\Land::where('region_id',$region_id->regionid)->where('is_merged_lot',0)->whereNull('parent_id')->count()}}</td>
        <td>{{round(\App\Models\Land::where('region_id',$region_id->regionid)->where('is_merged_lot',0)->whereNull('parent_id')->sum('area'),2)}}</td>
    ...
    @foreach($lands->Districts as $item)
        @php
            $counts = \App\Services\StatHelper::getOrganizationStatByRegion($region_id->regionid,$item->areaid);
            $count_inventory = \App\Services\StatHelper::getInventoryData($region_id->regionid,$item->areaid);
             $land_counts = \App\Services\StatHelper::getLandCountByRegion($region_id->regionid,$item->areaid);
               $land_suvli = \App\Services\StatHelper::GetSuvli($region_id->regionid,$item->areaid);
        @endphp
        <tr>
            <td>{{$item->nameuz}}</td>
